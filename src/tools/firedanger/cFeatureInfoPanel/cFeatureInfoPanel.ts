            ...
            if (Array.isArray(layerWithFeatureInfo.featureInfo)) {
              ...
              if (layerWithFeatureInfo.source?.vectorTile) {
                ...
                for (let featureInfoProperty in featureInfoNode) {
                  ...
                  // For Weekly NDVI Greenness layer we need to compute the pixel value differently.
                  // `modifyDisplayValue` is set to true in the layer config.
                  if (oneFeatureObject.modifyDisplayValue) oneFeatureObject.displayValue = (oneFeatureObject.displayValue - 100) / 100;
