  ...
  createExtendedTool: function (owningBlock) {
    return {
      ...
      generateTOC: function () {
        ...
        const overlays = globalThis.App.Layers.query(newLayersConfig.overlays, {
          type: 'layer',
          active: true,
          mask: false,
          loadOnly: false,
        ...
        for (; i < len; i += 1) {
          ...
          children.push({
            id: overlay.id,
            layerNodeTitle: `<span title="${overlayName}">${truncateString(overlayName, 0, 40)}</span>`, //overlayName.substr(0, 45) + " ...",
            //qtip: overlayName,
            name: overlayName,
            leaf: true,
            glpyh: 0xf1e0,
        ...
        for (let o in folders) {
          ...
          if (fdr.type == 'folder') {
            ...
            if (fdr.loadOnly == false) {
              const layerTitle = truncateString(fdr.title, 0, 40);

              if (fdr.timeseries != undefined) {
                globalThis.App.Periodicity.setPeriodType(fdr.timeseries.type);

                const dateText = globalThis.App.Periodicity.getTimeSeriesText(fdr.timeseries);
