...
import { truncateString, buildLabel, singleDigitToDouble } from '../../../helpers/string';
  ...
  createExtendedTool(owningBlock) {
    ...
    const extendedTool = {
      ...
      reconfigureDownloadTOC(): void {
        ...
        const config = globalThis.App.Layers.getLayersConfigById(globalThis.App.Layers.getConfigInstanceId());
        extendedTool.layers_with_extent = [];

        // why do we need this check?
        // if (extendedTool.categories[allCats[0]] !== undefined) {
        ...
        //   }
        // });
        versionConfig = config.overlays;

        // enableLayerMasking will determine layers and folders. Folders with zero layers are removed
        // hide layers that are not part of the current extent
        const newVersionConfig = extendedTool.enableLayerMasking(versionConfig);

        // we can control the values of everything in parse layers I think
        const overlays = this.parseLayers(newVersionConfig, titleLength);
      ...
      enableLayerMasking(versionConfig) {
        ...
        versionConfig.forEach((element) => {
          if (element.type === 'folder') {
            ...
            const layerCheckvalues = Object.values(extendedTool.categories);
            const layerCheckvaluesflat = layerCheckvalues.flat();
            const layerCheckvaluesflatfind = layerCheckvaluesflat.find(
              (item) =>
                element.name.includes(item[1]) || element.name.includes(item[3]) || element.id.includes(item[1]) || element.id.includes(item[3])
            );

            if (layerCheckvaluesflatfind) {
      ...
      extentTooBig(extent: Extent = this.extent) {
      ...
      toggleDownloadBtn(): void {
        if (this.emailIsValid(this.email) && true && this.extentIsValid() && this.bboxIsValid) {
      ...
      parseLayers: function (folders: Object, folderId: string, parentTitle = '') {
        const titleLength = owningBlock.blockConfig?.titleLength ?? 40;
        const TOCTree = [];
        let children;
        let includedExoticCategories = [];
        ...
        for (let o in folders) {
          ...
          if (fdr.type == 'folder') {
            if (
              fdr.title === 'Breakpoint Count' ||
              fdr.title === 'Slope of Linear Model' ||
              fdr.title === 'Year of Most Recent Break' ||
              fdr.title === 'RCMAP Basemap'
            ) {
              TOCTree.push({
                id: fdr.id,
                text: fdr.title,
                ...
                checked: false,
                belongsTo: folderId,
                downloadCat: fdr.downloadCat,
              });
            } else if (fdr.title === 'RCMAP Ecological Potential (EP)') {
              TOCTree.push({
                text: fdr.title, // what the users see
                id: fdr.id,
                expanded: false,
                checked: false,
                children: [
                  {
                    id: fdr.id + 'Departure',
                    text: 'Ecological Potential Cover Departure',
                    layerNodeTitle: `<span title="Ecological Potential Cover Departure">Ecological Potential Cover Departure</span>`,
                    leaf: true,
                    glpyh: 0xf1e0,
                    checked: false,
                    belongsTo: fdr.id,
                  ...
                  {
                    id: fdr.id + 'Layer',
                    text: 'Ecological Potential Cover',
                    layerNodeTitle: `<span title="Ecological Potential Cover">Ecological Potential Cover</span>`,
                    leaf: true,
                    glpyh: 0xf1e0,
                    checked: false,
                    belongsTo: fdr.id,
            ...
            } else if (fdr?.title?.includes('Percent Cover') || fdr?.title?.includes('Confidence')) {
              if (!includedExoticCategories.includes(fdr.folder[0].downloadCat)) {
                TOCTree.push({
                  id: fdr.folder[0].downloadCat,
                  text: fdr.folder[0].downloadCat,
                  layerNodeTitle: `<span title="${fdr.downloadCat}">${fdr.downloadCat}</span>`,
                  name: fdr.downloadCat,
                  leaf: true,
                  glpyh: 0xf1e0,
                  checked: false,
                  belongsTo: folderId,
                  downloadCat: fdr.folder[0].downloadCat,
                });
                includedExoticCategories.push(fdr.folder[0].downloadCat);
              }
            } else {
              children = this.parseLayers(fdr.folder, fdr.id, fdr.title);
            }
            ...
            if (fdr.loadOnly == false) {
              ...
              if (fdr.timeseries != undefined) {
                children = {
                  text: parentTitle, // what the user sees
                  id: fdr.id,
          ...
          if (fdr.type == 'folder' && !fdr.loadOnly && children.length) {
            if (children.length === 1) {
              // children[0].belongsTo = folderId;
              TOCTree.push(children[0]);
            } else {
              TOCTree.push({
                text: fdr.title, // what the users see
                id: fdr.id,
      ...
      parseXMLforCategories(request: XMLHttpRequest): void {
        ...
        for (let i = 0; i < categoryTags.length; i += 1) {
          ...
          allCats.indexOf(srcMeta) === -1 ? allCats.push(srcMeta) : null;
      ...
      updateBBoxCategories(): void {
        ...
        } else if (this.extentIsValid(extent) && this.extentTooBig(extent)) {
          Ext.getCmp('dataDownloadPanel').hide();
