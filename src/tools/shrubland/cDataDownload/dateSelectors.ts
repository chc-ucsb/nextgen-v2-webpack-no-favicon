      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            listeners: {
              checkchange(node, checked: boolean, item, index, e, eOpts) {
                let leafNode = node.raw.leaf;
                if (!leafNode) {
                  node.cascadeBy(function (node) {
                    node.set('checked', checked);
                  });
                } else if (!checked && node?.parentNode) {
                  node.parentNode.set('checked', checked);
                }
                this.selectedCategories = true;
                extendedTool.toggleDownloadBtn();
                const panel = Ext.getCmp('parentPanel');
                panel.body.scrollTo('top', extendedTool.scrollPosition);
              },
              beforeitemclick(treeModel, record, item, index, e, eOpts) {
                const panel = Ext.getCmp('parentPanel');
                extendedTool.scrollPosition2 = panel.body.getScrollTop();
              },
              afterlayout(treeModel, record, item, index, e, eOpts) {
                const panel = Ext.getCmp('parentPanel');
              ...
              },
              {
                ...
                listeners: {
                  click(): void {
                    ...
                    function compileRequestData(layerOfCategeory: Array<string>): { request: Object; fileNum: Number; noDataLayers: Boolean } {
                      const [
                      ...
                      const toDate = Ext.getCmp('toDate');
                      const mapProj = extendedTool.getCurrentMap().getView().getProjection().getCode();
                      const extent = extendedTool.getCurrentMapVectorSource().getExtent();
                      ...
                      let numOfFiles = 0; // keep track of how many files each category requests to download
                      let noDataLayers = false;
                      let srcFileNames: Array<string> | string = [];
                      let rqst: typeof RequestDataObj;
                      if (block.dateSelection === 'calendar') {
                        srcFileNames = extendedTool.getSourceFileNames(selectedSourceFolder, fromDate.value, toDate.value, selectedSourceFile);
                      } else if (block.dateSelection === 'yearCombos') {
                        srcFileNames = extendedTool.getSourceFileNames(
                          selectedSourceFolder,
                      ...
                      if (!extendedTool.extentIsValid(transExtent) && extendedTool.extentTooBig(transExtent)) {
                        alert('The bounding box of your download request is NOT VALID!');
                      }
                      if (srcFileNames?.length > 0) {
                        rqst = new RequestDataObj(
                          selectedClippedMetadataName,
                          selectedClippedRasterName,
                          selectedSourceFolder,
                          srcFileNames,
                      ...
                      return { request: rqst, fileNum: numOfFiles, noDataLayers: noDataLayers };
                    }

                    const downloadTree = Ext.getCmp('downloadTOCTree');
                    const noDataLayers = [];
                    const categoryArray = []; // an array of all categories the user has selected from
                    let numOfFiles = {}; // keep track of how many files each category requests to download
                    let requestData = [];

                    let categoryData: string;
                    const checked = downloadTree.getChecked();
                    for (let i in checked) {
                      // skip top level layers as checking them should auto check childNodes anyway
                      if (!checked[i]?.childNodes?.length) {
                        if (!categoryArray.includes(checked[i].raw.downloadCat)) {
                          categoryData = checked[i].raw.downloadCat;
                          if (!categoryData) {
                            continue;
                          }
                          ...
                          let crd;
                          let layerOfCategeory = extendedTool.categories[categoryData];
                          if (Array.isArray(layerOfCategeory)) {
                            for (let i = 0; i < layerOfCategeory.length; i++) {
                              crd = compileRequestData(layerOfCategeory[i]);
                              if (crd) {
                                if (crd?.request) requestData = requestData.concat(crd.request);
                                if (crd?.numOfFiles) numOfFiles[categoryData] = crd.numOfFiles;
                                if (crd?.noDataLayers && !noDataLayers.includes(categoryData)) noDataLayers.push(categoryData);
                              }
                            }
                          } else {
                            crd = compileRequestData(layerOfCategeory);
                            if (crd) {
                              if (crd?.request) requestData = requestData.concat(crd.request);
                              if (crd?.numOfFiles) numOfFiles[categoryData] = crd.numOfFiles;
                              if (crd?.noDataLayers && !noDataLayers.includes(categoryData)) noDataLayers.push(categoryData);
