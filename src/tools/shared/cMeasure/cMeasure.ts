...
export const cMeasure = {
  ...
  createExtendedTool(owningBlock) {
    ...
    const extendedTool = {
      ...
      formatLength(line) {
        ...
        const chosenDistance = Ext.getCmp('distanceCombo').value;
        switch (chosenDistance) {
          ...
          case 'Feet':
            ...
            output = `${Math.round(output * 3.281)} ft`;
            break;
          case 'Miles':
            output = Math.round((length / 1000) * 100) / 100;
            output = `${Math.round(output / 1.609)} mi`;
            break;
        }
        return output;
      },
      ...
      formatArea(polygon) {
        ...
        switch (chosenDistance) {
          ...
          case 'Feet':
            ...
            output = `${Math.round(output * 10.764)} ft<sup>2</sup>`;
            break;
          case 'Miles':
            output = Math.round((area / 1000000) * 100) / 100;
            output = `${Math.round(output / 2.59)} mi<sup>2</sup>`;
            break;
        }
        return output;
      },
      ...
      addInteraction(newValue) {
        if (newValue) {
          ...
          draw.on('drawend', function (evt) {
            ...
            extendedTool.measureTooltip.color = 'rgba(0, 0, 0, 0.7)';
  ...
  getComponent(extendedTool) {
    ...
    let distanceData = ['Miles', 'Kilometers', 'Meters', 'Feet'];
    const distanceStore = Ext.create('Ext.data.Store', {
      fields: ['value', 'text'],
      data: distanceData,
    });
    const component = {
      ...
      tools: [
        {
          ...
          callback: function (panel, tool, event) {
            Ext.Msg.show({
              title: 'Measure Tool',
              msg:
                'This tool allows the user to draw and measure an area or a line on the map. They can also choose the unit of measurement for their drawing. ',
      ...
      items: [
        {
          ...
          valueField: 'text',
          displayField: 'text',
          value: distanceData[0],
          width: 190,
          emptyText: 'Select Distance Unit',
          editable: false,
          style: { fontSize: '14px', fontWeight: 'bold', marginTop: '7px', marginBottom: '5px', marginLeft: '10px' },
          hidden: false,
          listeners: {
            beforerender(): void {
              const temp = ['Miles', 'Kilometers', 'Meters', 'Feet'];
              const distanceData = [];
              for (let i = 0, len = temp.length; i < len; i += 1) {
                const distanceConfig = temp[i];
                distanceData.push({
                  value: distanceConfig,
                  text: distanceConfig,
                });
              }
              distanceStore.removeAll();
              distanceStore.loadData(distanceData);
            },
            change(distanceCombo, value): void {
              const radioButtons = Ext.getCmp('measureRadio');
            },
          },
        },
        {
